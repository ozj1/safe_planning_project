...
ref_traj    = reftraj_gen(2*T, dt, -3*Lane_size/2.,20, 'LaneChange', aref,vref,tf,20);%it must be 3 for D1 and -3 for others 
...
for i = 1:1:NUM_TOTAL
    ...
    if y_temp_final ~= y_final%Omid: here we want to update  
        
    if i>NUM_CTRL/2 & i<NUM_TOTAL-NUM_CTRL/2
        traj_dy_dist = zeros(NUM_CTRL/2,1);
        sum_traj_dy_dist=0;
    for jj = i-NUM_CTRL/2:1:i-1
        nnn=sqrt(-4.5);
    ...
    ref_traj    = reftraj_gen(2*T, dt, -3*Lane_size/2.,20, 'LaneChange', aref,vref,tf,X_planned(1,i-1));%we need to update the x0 from 20 to current position of ego vehicle
...
%         plot(mid_road(:,1),mid_road(:,2),'r');  
    ...
    %         lp = plot(obs_long(1,:,i), obs_long(2,:,i), 'c*');
        ...
        xlim([0 250]); ylim([(road_low_lim-2) (road_up_lim+2)]);
