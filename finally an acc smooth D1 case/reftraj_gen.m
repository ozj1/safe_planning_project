function ref = reftraj_gen(T_tot, dt, y_init, varargin)
...
%     L = param.lf + param.lr;
    ...
    optargs = {'Linear' '' '' '3' '0'};%Omid: I changed {'Linear' '' '3' '20'} to {'Linear' '' '3' '0'}
...
%     x0 = str2num(cell2mat(optargs(5)));      ... initial x position
    ...
    if strcmp(type,'Linear')
        ...
        a_des=tgt_acc;
        interval  = v_des * dt;
%         whos x0 interval v_des dt
%         disp(isa(x0,'double'));

        if(isa(x0,'char'))%added by Omid
            x0 = str2double(x0);
        end
        x_ref     = x0:interval:(x0+interval*(NUM_TOTAL+NUM_CTRL));
    ...
    elseif strcmp(type,'LaneChange')
        ...
        elseif (y_init == -3*Lane_size/2.)
            y_final = -Lane_size/2.;%Omid: it should be -Lane_size/2. for one lane changing
        elseif (y_init == 3*Lane_size/2.)
            y_final = -3*Lane_size/2.;%Omid: it should be -Lane_size/2. for one lane changing
        end
        qy      = [y_init,0, 0, y_final, 0,     0]; 
