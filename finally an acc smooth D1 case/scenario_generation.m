...
% numbers
    ...
    if strcmp(Case_Code,'D1')
        ...
        cut_in_traj(:,:,1) = (reftraj_gen(T+T_horizon, dt, -3, 'LaneChange', 0,8, 3, 5))'; 
        obs1 = obs1.load_traj(cut_in_traj(:,1:1+NUM_CTRL,1), [], false);
        
        obs2 = obstacle([60;-3], [], 0,12, 0, NUM_CTRL, dt, t_switch);
        obs2 = obs2.update_ctrl(zeros(U_DIM, NUM_CTRL));
        % target performing lane change
        % last four inputs: (type, velocity, tf, x0)
        cut_in_traj(:,:,2) = (reftraj_gen(T+T_horizon, dt, -3, 'Linear', 0,8, 3, 60))'; 
        obs2 = obs2.load_traj(cut_in_traj(:,1:1+NUM_CTRL,2), [], false);
%         % obstacle2 
%         o2_init = [30; -Lane_size/2.; 0; 8; 0];
...
%         
%         cut_in_traj2(:,:,2) = o2_init * ones(1, (T+T_horizon)/dt);
% 
%         
% %         end
%         for i = 2:size(cut_in_traj2,2)
% disp(size(cut_in_traj2,2));
%             cut_in_traj2(1,i,2) = cut_in_traj2(1,i-1,2) + cos(cut_in_traj2(4,i-1,2)) * cut_in_traj2(3,i-1,2) * dt;
%             cut_in_traj2(2,i,2) = cut_in_traj2(2,i-1,2) + sin(cut_in_traj2(4,i-1,2)) * cut_in_traj2(3,i-1,2) * dt;
%         end
...
%             cut_in_traj(2,i,2) = cut_in_traj(2,i-1,2) + sin(cut_in_traj(4,i-1,2)) * cut_in_traj(3,i-1,2) * dt;
    ...
    elseif strcmp(Case_Code,'Od2')
        % obstacle1
        o1_init = [5;Lane_size/2.; 0; 8; 0];
        o1_vert = inflate(o1_init);
        o1      = obstacle(o1_init(1:2), o1_vert,o1_init(3), o1_init(4), o1_init(5), NUM_CTRL, dt, t_switch);
        o1      = o1.update_traj(false);
        
        % obstacle2 
        o2_init = [10; -Lane_size/2.; 0; 8; 0];
        o2_vert = inflate(o2_init);
        o2      = obstacle(o2_init(1:2), o2_vert, o2_init(3), o2_init(4), o2_init(5), NUM_CTRL, dt, t_switch);
        o2      = o2.update_traj(false);
        ...
        o3_init = [10;-3*Lane_size/2.; 0; 8;0];
        o3_vert = inflate(o3_init);
        o3      = obstacle(o3_init(1:2), o3_vert, o3_init(3), o3_init(4), o3_init(5), NUM_CTRL, dt, t_switch);
        o3      = o3.update_traj(false);
