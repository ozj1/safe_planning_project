...
global w_ref w_vel w_jerk w_acc w_del w_end_ref w_end_acc w_end_vel w_jerk_end w_del_end
...
%         g   = 1 - delX' * A * delX;   
        ...
        cost = cost + q1_back(i) * exp(q2_back(i) * g);
...
%         g   = 1 - delX' * A * delX;   
        ...
        % exponential barrier function
        cost = cost + q1_front(i) * exp(q2_front(i) * g);
        
        %%
        %added b Omid to test the new way of safety
        % this vector is not full state difference
%         if((Xi(1) - obs_pose(1))<0)
%         if(i<5)
%         Xi2 = X(:,i+10);
%         xc=5.;
%         delX = [Xi2(1) - obs_pose(1); Xi2(2) - obs_pose(2)];
% 
%         % this is the rotational matrix
%         rot_mat = [cos(obs_theta), -sin(obs_theta); 
%                    sin(obs_theta), cos(obs_theta)];
...
% %         g   = 1 - delX' * A * delX;   
%         % 'if' added by Omid as we don't want min_safe_dis to mess up with the vehicles on other lanes 
%         if (abs(Xi2(2) - obs_pose(2)) <= Lane_size/2)
%             g   = 1 +min_safe_dis- delX' * A * delX;   %min_safe_dis added by Omid
%         else
%             g   = 1 - delX' * A * delX;   %min_safe_dis added by Omid
%        end
%         % exponential barrier function
%         cost = cost + (q1_back * exp(q2_back * g))/xc;
%         
%         %% the front center of the ego vehicle
%         X_front = Xi2 + [cos(Xi2(5))*param.L; sin(Xi2(5))*param.L; 0;0; 0];
%         delX = [X_front(1) - obs_pose(1); X_front(2) - obs_pose(2)];
%         % scalar value inequality constraint
% %         g   = 1 - delX' * A * delX;   
%         % 'if' added by Omid as we don't want min_safe_dis to mess up with the vehicles on other lanes 
...
%        end
%         % exponential barrier function
%         cost = cost + (q1_front * exp(q2_front * g))/xc;
%         end
%         end
    ...
    % ============== road costraints ==============
    cost = cost + q1_road(i) * exp(q2_road(i) * (Xi(2) - (road_up_lim-param.wid/2.)));
    cost = cost + q1_road(i) * exp(q2_road(i) * ((road_low_lim+param.wid/2.) - Xi(2)));
    ...
    % upper bound
    g   = Xi(3) - v_dot_max;
    cost = cost + q1_acc(i) * exp(q2_acc(i) * g);
...
%     plot(cost,Xi(2));
    ...
    % upper bound
    g   = Ui(1) - a_dot_max;
    cost = cost + q1_jerk(i) * exp(q2_jerk(i) * g);
    ...
    % upper bound
    g   = Ui(2) - delta_max;
    cost = cost + q1_del(i) * exp(q2_del(i) * g);
...
cost = cost + [w_jerk(i) w_del(i)]*Ui.^2;%edited by Omid
...
cost = cost + [w_jerk_end w_del_end]*U(:,end).^2;%edited by Omid
