...
%         disp(obstacle(k).traj(1,i))
        ...
        E=dx_2-dy_t;
        mean_acc=0;sigma_acc=6;mean_jerk=0;sigma_jerk=0.1;
        ...
        w_acc(i)=(75.2*(1/(sigma_acc*(2*pi)^0.5))*(exp(-((D-mean_acc)^2)/(2*sigma_acc^2))))*(1+2*tanh(0.2*(dy_f+dx_2+V)))*(-log(1.00673-exp(-0.01*dy_t)));
