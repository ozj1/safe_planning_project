...
theta        = Xi(5);
x_front      = Xi + [cos(theta)*param.L; sin(theta)*param.L; 0;0; 0];
...
%%
%this part i added by Omid to test an idea (compaing i+10 of go with i of target vehicle)
...
%            sin(obs_theta),  cos(obs_theta)];
% 
% % inequality constraint value intermediate matrix that is state independent
% A   = rot_mat*diag([1/ellip_a^2, 1/ellip_b^2])*rot_mat';
% 
% %% the rear center of the ego vehicle
% % scalar value inequality constraint 'if' added by Omid as we don't want
% % min_safe_dis to mess up with the vehicles on other lanes
...
% ddb = q1_back * q2_back ^2 * exp(q2_back * g) * (dg) * (dg');   % scalar * (4x1) * (1x4)
% 
% cx = cx + db/xc;%Omid: I made it's influnce only 20% of real collision to avoid forgetting the reality
% cxx = cxx + ddb/xc;
% 
% %% the front center of the ego vehicle
% theta        = Xi2(5);
% x_front      = Xi2 + [cos(theta)*param.L; sin(theta)*param.L; 0;0; 0];
% 
% delX = [x_front(1) - obs_pose(1); x_front(2) - obs_pose(2)];
% 
% 
...
% % end
% 
% % if (abs(X(2) - obs_pose(2)) <= Lane_size/2) if (abs(X(1) - obs_pose(1))
% % <= 8)
% %     g=g;
% % end end
% 
% % 'if' added by Omid as we don't want min_safe_dis to mess up with the
