...
T_horizon = 2;      ... sliding horizon length
...
vref        = 15;
aref=0.;
ref_traj    = reftraj_gen(2*T, dt, 3*Lane_size/2., 'LaneChange', aref,vref);%it must be 3 for D1 and -3 for others 
top_lane    = reftraj_gen(2*T, dt,  road_up_lim, 'Linear', aref,vref);
side_lane   = reftraj_gen(2*T, dt, road_low_lim, 'Linear', aref, vref);
...
X_start(2)  = 3*Lane_size/2.;   ... initial y offset of -3m (on the ref trajectory)%it must be 3 for D1 and -3 for others 
X_start(4)  = 10;   ... initial velocity of 14.0m/sec
    ...
    if use_prediction == 1
        ...
        for k = 1:length(OBS)
            ...
            elseif i == 2
                ...
                BOUND = ones(3, 1);
                [x_long_term, ahat, H, VAR] = adaptive_prediction(x, x_old, ahat_old, H, VAR_old, NUM_CTRL);
            else
...
%     y(i,:) = inflated_car_state(2,:,i);
    ...
    % repeat procedure for,j target obstacle
    for j = 1:length(OBS)
...
if plot_gif == 1
    ...
    %     hold on
        p = plot(X_local(1,1:NUM_CTRL,i), X_local(2,1:NUM_CTRL,i), 'b--*');%this shows the the future position of ego vehicle as well
        hold on
...
%              for jj = 1:NUM_CTRL% added by Omid
             lp(hh) = plot(obs_long(1,:,i,hh), obs_long(2,:,i,hh), 'c*');
%              end
            end
            
        end

%         xlim([0 250]); ylim([(road_low_lim-2) (road_up_lim+2)]);%Omid I
%         have made this comment as it is not letting us to see the outside
%         of road
...
yyaxis left
for k = 1:length(OBS)
plot(obs_traj(4,:,k), 'm', 'LineWidth', 2);
    hold on
end
