...
if (adaptive == 0)
    ...
    global a_vel b_vel a_ref b_ref a_acc b_acc a_road b_road a_del
end
...
%for case D1
%         w_ref(i) = a_ref.*(obstacle(index).traj(4,i)/vref)/exp(b_ref * dx_wei.^0.75);
%         w_acc(i) = (obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
%         w_jerk(i) = 0.05*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^0.75);
%         w_del(i) = (obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
%         q1_jerk(i) = 0.05.*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^0.75);
%         q2_jerk(i) = q1_jerk(i) ;
%         q1_del(i) = (obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^0.75);
%         q2_del(i) = q1_del(i) ;
%         q1_acc(i) = (obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^0.75);
%         q2_acc(i) = q1_acc(i) ;
%         q1_back(i) = exp(b_acc * dx_wei.^0.75)/(obstacle(index).traj(4,i)/vref) ;
...
%         q2_road(i) = q1_road(i);
        ...
        w_ref(i) = a_ref.*(obstacle(index).traj(4,i)/vref)/exp(b_ref * dx_wei.^1.);
        w_acc(i) = a_acc*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        w_jerk(i) = 100.*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        w_del(i) = a_del*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        q1_jerk(i) = 100.*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        q2_jerk(i) = q1_jerk(i) ;
        q1_del(i) = a_del*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        q2_del(i) = q1_del(i) ;
        q1_acc(i) = (obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        q2_acc(i) = q1_acc(i) ;
        q1_back(i) = exp(b_acc * dx_wei.^1.)/(obstacle(index).traj(4,i)/vref) ;
        q2_back(i) = q1_back(i); q1_front(i) = q1_back(i); q2_front(i) = q1_back(i);
        q1_road(i) = a_road*q1_back(i);
        q2_road(i) = q1_road(i);
        else           
        w_ref(i) = a_ref.*(obstacle(index).traj(4,i)/vref)/exp(b_ref * dx_wei.^1.);
        w_vel(i) = exp(b_vel * dx_wei.^1.)/(obstacle(index).traj(4,i)/vref) ;
        w_acc(i) = a_acc*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        w_jerk(i) = 1.*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        w_del(i) = a_del*(obstacle(index).traj(4,i)/vref)/ exp(b_acc * dx_wei.^1.);
        ...
        q1_jerk=20* ones(NUM_CTRL,1);
        q2_jerk=20* ones(NUM_CTRL,1);
        q1_acc = 10* ones(NUM_CTRL,1);
        q2_acc = 10* ones(NUM_CTRL,1);
        q1_del = 20* ones(NUM_CTRL,1);
        q2_del = 20* ones(NUM_CTRL,1);


        q1_back     = 20* ones(NUM_CTRL,1);
        q2_back     = 20* ones(NUM_CTRL,1);

        q1_front    = 20* ones(NUM_CTRL,1);
        q2_front    = 20* ones(NUM_CTRL,1);

        q1_road = 20* ones(NUM_CTRL,1);
        q2_road = 20* ones(NUM_CTRL,1);
        end
...
%         w_vel(i) =  (obstacle(index).traj(4,i)/vref)/exp(b_vel * dx_wei.^0.25);
        w_vel(i) =  a_vel*exp(b_vel * dx_wei.^0.75)/(obstacle(index).traj(4,i)/vref) ;
