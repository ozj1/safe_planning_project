...
%vehicle objects
egoV=vehicle;
ref_traj_fractions=0.*ones(NUM_TOTAL,5);ref_start_index=1;
...
X_start(4)  = ego_initial_v;   ... initial velocity of 14.0m/sec
...
for i = 1:1:NUM_TOTAL
   ...
   if (i>2 && i<=NUM_CTRL/5) || destination_verified==1
...
%         hold on
        ...
        for j = 1:length(OBS)
            ...
            hold on
            if cut_in_traj(1,i,j)>0 && cut_in_traj(1,i,j)<NUM_TOTAL+50 && cut_in_traj(4,i,j)~=0
...
grid on
% legend([h1 h2 h3],{'target velocity', 'ego velocity', 'ego acceleration', 'FontSize',25, 'Location', 'northwest'});

 legend('target velocity', 'ego velocity', 'ego acceleration', 'FontSize',25, 'Location', 'northwest');
yyaxis left
if length(OBS)~=0
    for k = 2:length(OBS)
    h=plot(obs_traj(4,:,k), '-m', 'LineWidth', 2);
