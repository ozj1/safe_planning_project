...
%         else
       ...
       index=-1;Vindex=-1;front_index=1.5;back_index=1.5;%initalization index, we need to evaluate each time the nvironment to find the correct trget vehicle and if there's no car available in our lane index should be NaN 
...
%         end
         ...
         elseif index==-1 && Vindex==-1
             ...
             B=dx_1+vref_road-0.6*vref-2*param.len;
             C=dx_1+vref_road-vref-10*param.len;
              vref=floor(vref_road+abs((vref_road+0.1-vref_road)*tanh(0.01*dx_1+0.2*dx_2)));
         else
            ...
            vref=floor(obstacle(Vindex).traj(4,i)+abs((vref_road+0.1-obstacle(Vindex).traj(4,i))*tanh(0.01*dx_1+0.2*dx_2)));%+0.2*dx_2
            %vref=15.;
...
%          w_acc(i)=(37.59*(1/(sigma_acc*(2*pi)^0.5))*(exp(-((D-mean_acc)^2)/(2*sigma_acc^2))))*(1+2*tanh(0.2*(dy_f+dx_2+V)))*(-log(1.00673-exp(-0.01*dy_t)));
         ...
         w_jerk(i)=0.00005+2.5*(1/(sigma_jerk*(2*pi)^0.5))*(exp(-((F-mean_jerk)^2)/(2*sigma_jerk^2)));
