...
%         P1=0;P2=0;P3=0;P4=0;P5=0;P6=0;
        ...
        for j = 1:length(OBS)
            ...
            hold on
            if cut_in_traj(1,i,j)<NUM_TOTAL+50
            if ((cut_in_traj(2,i,j)==(road_up_lim-Lane_size/2.))) 

            pOBS(1,j)=text(cut_in_traj(1,i,j)-3,cut_in_traj(2,i,j)+10, '\DeltaV=' );
            hold on
            pOBS(2,j)=text(cut_in_traj(1,i,j)+3,cut_in_traj(2,i,j)+10, num2str(cut_in_traj(4,i,j)-vref_road) );
            hold on
            pOBS(3,j)=text(cut_in_traj(1,i,j)-3,cut_in_traj(2,i,j)+8, '\DeltaX(1)=' );
            hold on
            pOBS(4,j)=text(cut_in_traj(1,i,j)+5,cut_in_traj(2,i,j)+8, num2str(cut_in_traj(1,i,j)-X_planned(1,i),4) );
            hold on
