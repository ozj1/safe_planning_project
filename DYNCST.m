...
%        end
       ...
       if i<15
       % phase determination
         if mk>0 && (index==-1 || mainTCV>=vref_road) && frontTCV>=0.9*vref_road && ((Xi(1)-closest_back_car_dist)>1*param.len) && ((closest_front_car_dist-closest_back_car_dist)>8*param.len) % if mk=0 it means egopolicy2<0 meaning frst or last lane is not clear and safe  
